...
/**
 * Réordonne une rangée pour que chaque couple soit adjacent (AUDIT M1).
 * Sans ça, un frère peut rester entre deux époux dans l'ordre de placement.
 */
...
type UniteRang = {
  ids: string[];
  /** Centre géométrique de l'unité (moyenne des x). */
  centre: number;
};

/**
 * Construit les unités atomiques d'une rangée : un couple = un bloc,
 * le reste = singletons. Personne ne peut s'intercaler dans un couple.
 */
...
/**
 * Colle les conjoints et empile les unités sans intercalaire (AUDIT M1 / C2).
 * Travaille en unités de layout (pas encore en pixels).
 */
function rapprocherConjointsSurRang(
  ...
  for (const unite of unites) {
    if (unite.ids.length > 1) {
      ...
      const centreMin = Number.isFinite(bordDroit)
        ? bordDroit + ECART_FRATRIE_LAYOUT
        : unite.centre;
      const centre = Math.max(unite.centre, centreMin);
      positions.set(id, centre);
      bordDroit = centre;
    }
  }
}
...
function lienVisuelFamille(
  ...
  const rangFocus = rangs.get(racineId) ?? 0;
  if (rang === rangFocus && id !== racineId) return 'cousin';
  ...
  for (const rang of rangsTries) {
    ...
    for (const [index, id] of liste.entries()) {
      ...
      if (conjointsPlaces.length > 0) {
        ...
        positions.set(id, x);
        curseur = x;
        derniereUnion = unionParentaleDe(id)?.id ?? '_';
        continue;
      }
 ...
 * groupées, puis le reste par nom.
 */
function ordonnerCoucheEclate(
  couche: { id: string; lien: LienRacine }[],
  rang: number,
  personnes: DonneesArbre['personnes'],
  unions: DonneesArbre['unions'],
  place: Map<string, NoeudArbre>
): { id: string; lien: LienRacine }[] {
  const parId = new Map(couche.map((e) => [e.id, e]));
  const utilises = new Set<string>();
  const resultat: { id: string; lien: LienRacine }[] = [];

  const nom = (id: string) => personnes.get(id)?.nomComplet ?? '';

  const ajouter = (id: string) => {
    if (utilises.has(id)) return;
  ...
  const partenairePlace = (id: string): NoeudArbre | null => {
    const personne = personnes.get(id);
    ...
    for (const unionId of personne.unions ?? []) {
      ...
      for (const c of [union.conjointA, union.conjointB]) {
        if (!c || c === id) continue;
        const noeud = place.get(c);
        if (noeud && noeud.rang === rang - 1) return noeud;
      }
    }
    return null;
  };

  const fratrieDe = (id: string): string[] => {
    const issuDe = personnes.get(id)?.issuDe;
    if (!issuDe) return [];
    return (unions.get(issuDe)?.enfants ?? []).filter((f) => f !== id && parId.has(f));
  };

  const conjointsDe = (id: string): string[] => {
    const personne = personnes.get(id);
    if (!personne) return [];
    ...
    for (const unionId of personne.unions ?? []) {
      ...
      if (!union) continue;
      for (const c of [union.conjointA, union.conjointB]) {
        if (c && c !== id && parId.has(c)) trouves.push(c);
      }
    }
    return trouves;
  };

  const racine = couche.find((e) => e.lien === 'racine');
  if (racine) ajouter(racine.id);

  // Conjoints d'une personne déjà posée à la couche précédente.
  const conjointsLies = couche
    .filter((e) => e.lien === 'conjoint' && partenairePlace(e.id))
    .sort(
      (a, b) =>
        (partenairePlace(a.id)?.x ?? 0) - (partenairePlace(b.id)?.x ?? 0)
    );
  for (const entree of conjointsLies) ajouter(entree.id);

  // Fratries et paires de conjoints dans la couche.
  const groupes: string[][] = [];
  for (const entree of couche) {
    if (utilises.has(entree.id)) continue;
    const groupe = [entree.id];
    for (const frere of fratrieDe(entree.id)) {
      if (!utilises.has(frere) && parId.has(frere)) groupe.push(frere);
    }
    for (const conjoint of conjointsDe(entree.id)) {
      if (!utilises.has(conjoint) && parId.has(conjoint)) groupe.push(conjoint);
    }
    groupes.push([...new Set(groupe)].sort((a, b) => nom(a).localeCompare(nom(b), 'fr')));
  }

  groupes.sort((ga, gb) => nom(ga[0]!).localeCompare(nom(gb[0]!), 'fr'));
  for (const groupe of groupes) {
    for (const id of groupe) ajouter(id);
  }

  for (const entree of couche) ajouter(entree.id);

  return resultat;
}

/**
 * Écarte les unités atomiques (couples collés, singletons) sur chaque rangée.
 * Personne ne peut s'intercaler entre deux conjoints (AUDIT M1).
 */
  ...
  for (const liste of parRang.values()) {
    ...
    for (const unite of unites) {
      if (unite.ids.length > 1) {
        ...
        if (!n) continue;
        const xMin = Number.isFinite(bordDroit)
          ? bordDroit + ECART_MINIMUM_CENTRES
          : unite.centre;
        n.x = Math.max(unite.centre, xMin);
        bordDroit = n.x;
      }
    }
  }
...
/** Aligne le centre d'une fratrie sur le centre de ses parents visibles. */
function recentererFratriesSousCouples(
  noeuds: NoeudArbre[],
  unions: Map<string, UnionArbre>
): void {
  const parId = new Map(noeuds.map((n) => [n.personneId, n]));

  for (const union of unions.values()) {
    const parents = [union.conjointA, union.conjointB]
      .map((id) => (id ? parId.get(id) : undefined))
      .filter((n): n is NoeudArbre => n !== undefined);
