...
export type OptionPersonne = PortraitMentionnable;

/**
 * Qui est là-dedans.
 *
 * Cent-neuf personnes tiennent dans une liste, mais pas dans une tête : un
 * champ de recherche filtre au fur et à mesure, et chaque nom se déploie en
 * Vignette dès qu’on le survole. Sélectionnée, la personne reste visible sous
 * la même forme — impossible alors de la confondre avec un homonyme.
 */
export function ChoixPersonnes({
  ...
  valeurs?: string[];
  /** Signalé à la prévisualisation vivant à côté du formulaire. */
  onChangement?: (personnes: OptionPersonne[]) => void;
}) {
    ...
    <fieldset className="flex flex-col gap-3 rounded-[var(--rayon)] border border-bordure p-4">
          ...
          {selectionnees.map((personne) => (
            <li key={personne.id} className="flex items-stretch gap-2">
              <div className="min-w-0 flex-1">
                <Vignette personne={portraitDePortrait(personne)} />
              </div>
              <button
                type="button"
      ...
      <div className="grid gap-3 sm:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]">
        ...
        <div className="min-h-16">
          ...
          {apercu ? (
            <Vignette personne={portraitDePortrait(apercu)} />
          ) : (
...
function anneesCourtes(p: OptionPersonne): string {
