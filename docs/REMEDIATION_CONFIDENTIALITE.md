...
3. Vérifier si le projet Supabase héberge d’autres applications. S’il est partagé, exclure leur schéma et leur historique de toute action : ni réinitialisation du projet ni remplacement de l’historique global ne sont admissibles.
4. Séparer le schéma reproductible des imports de données familiales : le dépôt public ne doit conserver que le schéma, les RLS, les fonctions et des exemples fictifs.
5. Préparer sur une copie isolée la réconciliation entre l’historique de migrations de la production et le nouveau dépôt assaini. Ne jamais modifier ou rejouer une migration historique directement contre la production.
6. Faire relire le plan par le propriétaire, puis exécuter la purge d’historique avec un responsable identifié. Informer les collaborateurs qu’un nouveau clonage est nécessaire et faire révoquer les accès ou secrets concernés si l’inventaire le justifie.
7. Vérifier sur une cible isolée que le schéma, les politiques RLS, les médias privés et l’application restent cohérents ; consigner le résultat hors du dépôt si des données réelles sont impliquées.
8. Après assainissement validé, ajouter à la CI un contrôle de confidentialité qui analyse les ajouts de chaque changement, sans ignorer les chemins sensibles nouvellement modifiés.
